...
func (db *productDb) GetPromoted() ([]int64, error) {
	conn := getConn()
	defer conn.Close(context.Background())
	...
	rows, err := conn.Query(
		context.Background(),
		"SELECT \"id\" FROM products WHERE promoted = true")

	if err != nil {
		return productsIds, err
	}
...
func (db *productDb) StartsWith(search string) ([]int64, error) {
	conn := getConn()
	defer conn.Close(context.Background())
